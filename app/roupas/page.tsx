...
export default function RoupasPage() {
  ...
  useEffect(() => {
    async function buscarRoupas() {
      try {
        // 1. Aponta para a API única pedindo roupas
        const response = await fetch('/api/produtos?categoria=roupas');
        if (!response.ok) throw new Error("Erro na requisição");
        
        const dados = (await response.json()) as Produto[];
        
        // 2. Trava de segurança configurada para aceitar ROUPAS
        const apenasRoupas = dados.filter(produto => {
          if (!produto.categoria) return false;
          const cat = produto.categoria.toLowerCase().trim();
          return cat === 'roupas' || cat === 'roupa';
        });

        setTodosProdutos(apenasRoupas);
        setProdutosFiltrados(apenasRoupas);
      } catch (error) {
    ...
    <div className="w-full bg-zinc-50 min-h-screen pb-24 text-black">
      <section className="w-full bg-white border-b border-zinc-200 py-6 px-4 shadow-sm">
        <div className="max-w-7xl mx-auto flex flex-col gap-4">
          <div>
            <h1 className="text-xl font-black uppercase tracking-tight text-zinc-900">Roupas</h1>
            <p className="text-xs text-zinc-500">
              {termoBusca ? `Buscando roupas por: "${termoBusca}"` : "Renove seu estilo com nossa coleção de vestuário."}
            </p>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-2 sm:px-4 py-6">
        {produtosFiltrados.length === 0 ? (
          <div className="text-center py-12 bg-white rounded-xl border p-6">
            <p className="text-zinc-500 text-sm">Nenhuma roupa encontrada com esses filtros.</p>
          </div>
        ) : (
